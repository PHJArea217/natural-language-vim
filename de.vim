...
syn keyword dePronounsPersonal ich du er sie es wir ihr mich dich ihn uns euch mir dir ihm ihnen sich
syn match deEinPronouns "\<\([mdsk]\?ein\|ihr\|unse\?r\|eue\?r\)\(e[rsnm]\?\)\?\>"
syn match dePronounsImpersonal "\<\(irgend\)\?\(jemand\(e[rsnm]\?\)\?\|etwas\|wo\(her\|hin\)\?\)\>" " etwas, irgendwohin, etc.
...
syn keyword dePronounsImpersonal nichts
...
syn match dePronounsOther "\<\(da\|dort\)\(her\|hin\)\?\>"
...
syn keyword dePrepositions ab an auf aufs aus außer außerhalb ausser ausserhalb bei beim durch durchs entgegen entsprechend
syn keyword dePrepositions für fürs gegen gegens hinter hinterm hintern in im ins innerhalb mit nach neben nebst ohne pro
syn keyword dePrepositions seit statt trotz über überm übers um ums unter unters von vom vor während wegen zu zur zum

syn keyword deConjunctionsCoord und oder aber denn dennoch entweder doch so
syn keyword deConjunctionsSubord anstatt bevor weil dass hingegen indem inder indessen nachdem ob obgleich obwohl seitdem wenn zudem zuder
syn match deConjunctionsSubord "\<so\(bald\|wie\|lange\?\|ndern\|viel\|wohl\|weit\)\>"

syn keyword deDefiniteArticle die das der den dem des

syn match deContractions "'[snm]\>"

syn match deNumbers "\<\(eins\|zwei\|drei\|sechs\|sieben\|\(vier\|fünf\|sech\|sieb\|acht\|neun\)\(zehn\|zig\)\?\|zehn\|elf\|zwölf\|dreizehn\|dreißig\|dreissig\|hundert\|und\|tausend\|million\|milliard\|billion\|billiard\|trillion\)\+\>"
